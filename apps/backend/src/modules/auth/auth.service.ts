...
export class AuthService {
  ...
  async validateUser(email: string, pass: string): Promise<SafeUser> {
    ...
    if (!isMatch) throw new UnauthorizedException("The password provided is incorrect");
  ...
  async verifyEmail(token: string) {
    ...
    await this.db
      .update(users)
      .set({ emailVerified: new Date(), verificationToken: null, verificationTokenExpiry: null })
      .where(eq(users.id, user.id));
